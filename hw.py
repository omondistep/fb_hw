...
class PendingPrediction:
    """Store predictions awaiting verification"""
    match_id: str
    url: str
    timestamp: datetime
    home_team: str
    ...
    match_characteristics: List[str]
    verification_time: datetime  # When to check for results
...
class AutoVerificationSystem:
    """Automatically verifies predictions by re-checking URLs"""
    
    def __init__(self, db_file="predictions_tracker.db"):
        self.db_file = db_file
        self.pending_predictions: Dict[str, PendingPrediction] = {}
    ...
    def init_database(self):
        ...
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS predictions (
                ...
                predicted_probabilities TEXT,
                confidence_score INTEGER,
                key_factors TEXT,
                match_characteristics TEXT,
    ...
    def save_prediction(self, prediction: PendingPrediction):
        ...
        cursor.execute('''
            ...
            (match_id, url, timestamp, home_team, away_team, predicted_result,
             predicted_probabilities, confidence_score, key_factors, 
             match_characteristics, verification_time, verification_attempts, status)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (
            prediction.match_id,
            prediction.url,
            ...
            json.dumps(prediction.match_characteristics),
            prediction.verification_time.isoformat(),
            prediction.verification_attempts,
            prediction.status
        ))
        
        conn.commit()
        conn.close()
        self.pending_predictions[prediction.match_id] = prediction
    ...
    def load_pending_predictions(self):
        ...
        cursor.execute('''
            ...
            AND verification_time <= datetime('now', '+1 day')
        ''')
        ...
        for row in rows:
            prediction = PendingPrediction(
                ...
                predicted_probabilities=json.loads(row[6]),
                confidence_score=row[7],
                key_factors=json.loads(row[8]),
                match_characteristics=json.loads(row[9]),
                verification_time=datetime.fromisoformat(row[10]),
                verification_attempts=row[11],
                status=row[12]
            )
            self.pending_predictions[prediction.match_id] = prediction
    ...
    def record_prediction_for_verification(self, match_data: Dict, url: str):
        """Record a prediction for automatic verification"""
        
        match_id = self.generate_match_id(url)
        verification_time = datetime.now() + timedelta(hours=26)  # Check after 26 hours
        
        # Get predicted result from decisions
        decisions = match_data.get('decisions', {})
        predicted_result = 'U'
        if decisions.get('primary_bet'):
            selection = decisions['primary_bet']['selection']
        ...
        # Safely extract match_summary characteristics
        match_summary = match_data.get('match_summary')
        characteristics = []
        if match_summary:
            if hasattr(match_summary, 'characteristics'):
                characteristics = match_summary.characteristics[:5]
            elif isinstance(match_summary, dict):
                characteristics = match_summary.get('characteristics', [])[:5]
        
        # Safely extract core_analysis key_factors
        core_analysis = match_data.get('core_analysis')
        key_factors = []
        if core_analysis:
            if hasattr(core_analysis, 'key_factors'):
                key_factors = core_analysis.key_factors[:5]
            elif isinstance(core_analysis, dict):
                key_factors = core_analysis.get('key_factors', [])[:5]
        ...
        prediction = PendingPrediction(
            ...
            predicted_probabilities=match_data.get('probabilities', {}),
            confidence_score=decisions.get('confidence_score', 0),
            key_factors=key_factors,
            match_characteristics=characteristics,
            verification_time=verification_time
        )
        
        self.save_prediction(prediction)
        print(f"\n📝 Prediction recorded for auto-verification at {verification_time.strftime('%Y-%m-%d %H:%M')}")
    
    def clean_team_name_display(self, name: str) -> str:
        """Clean team name for display and matching"""
    ...
    def extract_actual_result(self, html: str, home_team: str, away_team: str) -> Optional[str]:
        """Extract actual result from HTML after match is played - IMPROVED"""
        
        soup = BeautifulSoup(html, 'html.parser')
        
        # Clean team names for matching
        home_clean = self.clean_team_name_display(home_team).lower()
        away_clean = self.clean_team_name_display(away_team).lower()
        
        # Try multiple extraction strategies
        
        # STRATEGY 1: Look for score in common Forebet patterns
        score_patterns = [
            # Pattern for "FT 1-0" or similar
            r'(?:FT|HT|Result|Final)\s*[:\-]?\s*(\d+)\s*[–\-:\s]\s*(\d+)',
            # Pattern for "(1-0)" with FT context
            r'\((\d+)[\-\–](\d+)\)\s*\(?FT\)?',
            # Pattern in score tables
            r'<span[^>]*>\s*(\d+)\s*</span>\s*[–\-]\s*<span[^>]*>\s*(\d+)\s*</span>',
            # Common score display
            r'class="[^"]*score[^"]*"[^>]*>\s*(\d+)\s*[–\-]\s*(\d+)\s*<',
        ]
        ...
        # STRATEGY 2: Look for result in structured data
        result_indicators = [
            ('class="won"', 'home_team'),  # Home won
            ('class="lost"', 'away_team'),  # Away lost (meaning home won)
            ('"result":"H"', 'H'),
            ('"result":"A"', 'A'),
            ('"result":"D"', 'D'),
            ('Home win', 'H'),
        ...
        for indicator, result in result_indicators:
            if indicator.lower() in html_lower:
                if result == 'home_team':
                    # Check if this is actually for the home team
                    home_pos = html_lower.find(home_clean[:15].lower())
                    indicator_pos = html_lower.find(indicator.lower())
                    if home_pos > 0 and abs(home_pos - indicator_pos) < 100:
                        return 'H'
                elif result == 'away_team':
                    away_pos = html_lower.find(away_clean[:15].lower())
                    indicator_pos = html_lower.find(indicator.lower())
                    if away_pos > 0 and abs(away_pos - indicator_pos) < 100:
                        return 'A'
                else:
                    return result.upper() if result in ['H', 'A', 'D'] else result
        
        # STRATEGY 3: Parse the page with BeautifulSoup for specific elements
        # Look for score in common Forebet divs
        score_selectors = [
            'div.score',
            'span.score',
            'div.result',
        ...
        # STRATEGY 4: Look for final result in text
        # Common patterns in football results
        final_patterns = [
            r'Final\s+Score[:\s]*(\d+)\s*[–\-]\s*(\d+)',
            r'Result[:\s]*(\d+)\s*[–\-]\s*(\d+)',
            r'Full[-\s]*Time[:\s]*(\d+)\s*[–\-]\s*(\d+)',
            r'FT[:\s]*(\d+)\s*[–\-]\s*(\d+)',
            r'Match[-\s]*Result[:\s]*(\d+)\s*[–\-]\s*(\d+)',
        ]
        ...
        # STRATEGY 5: Fallback - search for team names near scores
        # Find all numbers that look like scores (pairs of 0-5 usually)
        score_matches = re.findall(r'\b(\d+)\s*[–\-]\s*(\d+)\b', html)
        for score_match in score_matches:
            try:
                score1 = int(score_match[0])
                score2 = int(score_match[1])
                
                # Check if this score appears near team names
                score_text = f"{score1}-{score2}"
                score_pos = html.find(score_text)
                
                if score_pos > 0:
                    # Get context around the score
                    start = max(0, score_pos - 200)
                    end = min(len(html), score_pos + 200)
                    context = html[start:end].lower()
                    
                    # Check which team is mentioned first in context
                    home_in_context = home_clean[:15] in context
                    away_in_context = away_clean[:15] in context
                    
                    if home_in_context and not away_in_context:
                        # Home team mentioned, assume they're first
                        return 'H' if score1 > score2 else 'A' if score2 > score1 else 'D'
                    elif away_in_context and not home_in_context:
                        # Away team mentioned, assume they're first
                        return 'A' if score1 > score2 else 'H' if score2 > score1 else 'D'
                    
            except ValueError:
                continue
        
        # STRATEGY 6: Check for "defeated", "beat", etc. with team names
        defeat_patterns = [
            (r'defeated\s+([^,.<]+)', 'H'),  # Home defeated [Away]
            (r'beat\s+([^,.<]+)', 'H'),      # Home beat [Away]
            (r'([^,.<]+)\s+defeated', 'A'),  # [Away] defeated Home
            (r'([^,.<]+)\s+beat', 'A'),      # [Away] beat Home
        ]
        
        for pattern, expected_result in defeat_patterns:
            matches = re.findall(pattern, html, re.IGNORECASE)
            for team_text in matches:
                team_text_lower = team_text.lower()
                if home_clean in team_text_lower or any(word in team_text_lower for word in home_clean.split()):
                    return 'A' if expected_result == 'H' else 'H'
                elif away_clean in team_text_lower or any(word in team_text_lower for word in away_clean.split()):
                    return expected_result
        
        # STRATEGY 7: If nothing found, return None
        return None
    
    def verify_prediction(self, prediction: PendingPrediction) -> bool:
        """Verify a single prediction by re-checking the URL"""
        
        print(f"\n🔍 Verifying prediction for {prediction.home_team} vs {prediction.away_team}")
        print(f"   Predicted: {prediction.predicted_result}")
        
        # Fetch the page again
        try:
            # Use the existing fetch method from EnhancedForebetAnalyzer
            response = requests.get(prediction.url, timeout=15, headers={
                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'
            })
            html = response.text
            
            # Look for any score patterns for debugging
            score_patterns = re.findall(r'\b(\d+)\s*[–\-]\s*(\d+)\b', html)
            print(f"   Debug: Found {len(score_patterns)} score patterns on page")
            if score_patterns:
                print(f"   Debug: Sample scores: {score_patterns[:3]}")
            
            # Look for FT text
            if 'FT' in html.upper():
                print(f"   Debug: 'FT' found in HTML")
            
            actual_result = self.extract_actual_result(html, prediction.home_team, prediction.away_team)
            
            if not actual_result:
                print(f"   ⚠️ Result not found yet")
                
                # Try a simpler approach - look for common result patterns
                simple_patterns = [
                    r'>\s*FT\s*[:\-]?\s*(\d+)\s*[–\-]\s*(\d+)\s*<',
                    r'>\s*Result\s*[:\-]?\s*(\d+)\s*[–\-]\s*(\d+)\s*<',
                    r'>\s*(\d+)\s*[–\-]\s*(\d+)\s*\(FT\)',
                ]
                ...
                for pattern in simple_patterns:
                    ...
                    if match:
                        try:
                            home_goals = int(match.group(1))
                            away_goals = int(match.group(2))
                            print(f"   Debug: Simple pattern found: {home_goals}-{away_goals}")
                ...
                if not actual_result:
                    ...
                    with open(f"debug_{prediction.match_id}.html", "w", encoding="utf-8") as f:
                        f.write(html[:5000])  # First 5000 chars
                    print(f"   Debug: Saved HTML to debug_{prediction.match_id}.html")
                    
                    prediction.verification_attempts += 1
                    
                    if prediction.verification_attempts >= 3:
                        prediction.status = "failed"
                        print(f"   ❌ Max verification attempts reached")
                    else:
                        # Retry in 6 hours
                        prediction.verification_time = datetime.now() + timedelta(hours=6)
                        print(f"   ⏰ Will retry in 6 hours (attempt {prediction.verification_attempts}/3)")
                    
                    self.update_prediction_status(prediction)
                    return False
            
            # We have a result!
            print(f"   ✅ Found result: {actual_result}")
            print(f"   Predicted: {prediction.predicted_result}, Actual: {actual_result}")
            
            # Update prediction record
            prediction.status = "verified"
            prediction.actual_result = actual_result
            prediction.was_correct = (prediction.predicted_result == actual_result)
            ...
            # Record for learning
            self.record_for_learning(prediction)
            
            return True
            
        except Exception as e:
            print(f"   ❌ Error fetching: {str(e)[:50]}")
            prediction.verification_attempts += 1
            self.update_prediction_status(prediction)
            return False
    
    def update_prediction_status(self, prediction: PendingPrediction):
    ...
    def record_for_learning(self, prediction: PendingPrediction):
        """Record verified prediction for model learning"""
        ...
        outcome = PredictionOutcome(
            ...
            key_factors_used=prediction.key_factors,
            match_characteristics=prediction.match_characteristics,
            was_correct=prediction.was_correct,
            error_magnitude=self.calculate_error_magnitude(prediction)
        )
        
        self.learning_engine.record_prediction(outcome)
        print(f"   📚 Result recorded for learning (Correct: {prediction.was_correct})")
    
    def calculate_error_magnitude(self, prediction: PendingPrediction) -> float:
        """Calculate how wrong the prediction was"""
        if prediction.was_correct:
            return 0.0
        
        # Get predicted probability for the wrong outcome
        pred_prob = prediction.predicted_probabilities.get({
            'H': 'home_win',
            'A': 'away_win',
            'D': 'draw'
        }.get(prediction.predicted_result, 'home_win'), 50)
        
        return pred_prob / 100
    ...
    def run_verification_cycle(self):
        """Run one verification cycle for all pending predictions"""
        print(f"\n🔄 Auto-verification cycle at {datetime.now().strftime('%H:%M:%S')}")
        
        current_time = datetime.now()
        verified_count = 0
        
        for match_id, prediction in list(self.pending_predictions.items()):
            if prediction.verification_time <= current_time and prediction.status == "pending":
                try:
                    success = self.verify_prediction(prediction)
                    if success:
                        verified_count += 1
                        # Remove from pending list
                        del self.pending_predictions[match_id]
                except Exception as e:
    ...
    def generate_learning_report(self):
        ...
        report = self.learning_engine.get_performance_report()
        
        print(f"\n📊 LEARNING REPORT")
        print(f"Total Predictions: {report['total_predictions']}")
        print(f"Accuracy: {report['accuracy']}")
        print(f"Recent Accuracy: {report['recent_accuracy']:.1%}")
    
    def start_auto_verification(self):
        """Start automatic verification in background"""
        print("🚀 Starting auto-verification system...")
        
        # Run immediately
        self.run_verification_cycle()
        
        # Run every 4 hours in background
        def scheduler_loop():
            schedule.every(4).hours.do(self.run_verification_cycle)
        ...
        print("✅ Auto-verification running in background")
...
class DecisionEngine:
    ...
    def compute_metrics_from_matches(self, team_name: str, matches: List[Match], 
        ...
        recent_matches = min(3, total_matches)
        ...
        form_rating = points / (total_matches * 3) if total_matches > 0 else 0.5
    ...
    def calculate_probabilities_with_learning(self, home_metrics: TeamMetrics, 
                                            ...
                                            home_home_metrics: Dict = None,
                                            away_away_metrics: Dict = None,
                                            league: str = "Unknown"):
        ...
        if home_home_metrics and away_away_metrics:
            home_home_performance = home_home_metrics.get('home_form_rating', 0.5)
            away_away_performance = away_away_metrics.get('away_form_rating', 0.5)
            
            home_advantage_factor = home_home_performance * 0.6 + home_metrics.home_advantage * 0.4
            away_disadvantage_factor = (1 - home_home_performance) * 0.6 + (1 - home_metrics.home_advantage) * 0.4
    ...
    def make_decisions(self, probabilities: Dict, goal_markets: Dict,
                      ...
                      factors: MatchFactors,
                      home_home_metrics: Dict = None,
                      away_away_metrics: Dict = None) -> Dict:
        """Make betting decisions with enhanced logic and risk management including home/away analysis"""
        ...
        # 11. Home/Away specific performance insights
        if home_home_metrics and away_away_metrics:
            home_home_form = home_home_metrics.get('home_form_rating', 0.5)
            away_away_form = away_away_metrics.get('away_form_rating', 0.5)
            
            # Home team home strength
            if home_home_form > 0.7:
                confidence_score += 2
                decisions['home_away_insights'].append(f"{home_metrics.team_name[:12]} is STRONG at home ({home_home_form:.0%} win rate)")
            elif home_home_form < 0.3:
                confidence_score -= 1
                decisions['home_away_insights'].append(f"{home_metrics.team_name[:12]} is WEAK at home ({home_home_form:.0%} win rate)")
            
            # Away team away strength
            if away_away_form > 0.7:
                confidence_score += 2
                decisions['home_away_insights'].append(f"{away_metrics.team_name[:12]} is STRONG away ({away_away_form:.0%} win rate)")
            elif away_away_form < 0.3:
                confidence_score -= 1
                decisions['home_away_insights'].append(f"{away_metrics.team_name[:12]} is WEAK away ({away_away_form:.0%} win rate)")
        ...
        # EMERGENCY DRAW DETECTION - Critical fix for draw prediction blindness
            ...
            if match_summary.is_mismatch:
                ...
                # Home win decision - ADJUSTED FOR HIGH CONFIDENCE
                    ...
                    # Add home/away specific reasoning
                    if home_home_metrics and home_home_metrics.get('home_form_rating', 0.5) > 0.7:
                        primary_reasoning.append(f"Excellent home form ({home_home_metrics['home_form_rating']:.0%} win rate at home)")
                ...
                # Away win decision - ADJUSTED FOR HIGH CONFIDENCE  
                    ...
                    # Add home/away specific reasoning
                    if away_away_metrics and away_away_metrics.get('away_form_rating', 0.5) > 0.7:
                        primary_reasoning.append(f"Excellent away form ({away_away_metrics['away_form_rating']:.0%} win rate away)")
        ...
        # Home/Away specific warnings
        if home_home_metrics and home_home_metrics.get('home_form_rating', 0.5) < 0.3:
            decisions['avoid_bets'].append(f"WARNING: {home_metrics.team_name[:10]} has POOR home form")
        if away_away_metrics and away_away_metrics.get('away_form_rating', 0.5) < 0.3:
            decisions['avoid_bets'].append(f"WARNING: {away_metrics.team_name[:10]} has POOR away form")
        ...
        # Add home/away insights to reasoning
        if decisions['home_away_insights']:
            decisions['reasoning'].extend(decisions['home_away_insights'])
...
class EnhancedForebetAnalyzer:
    """Main analyzer class with learning capabilities"""
    ...
    def __init__(self):
        ...
        self.verification_system = AutoVerificationSystem()
    ...
    def extract_home_away_matches(self, html: str, team_name: str) -> Dict[str, List[Match]]:
        """Extract home and away matches separately from the HTML structure"""
        home_matches = []
        away_matches = []
        
        if not html or not team_name:
            return {"home": home_matches, "away": away_matches}
        
        # Look for "HOME MATCHES" and "AWAY MATCHES" sections
        home_section_pattern = r'HOME MATCHES.*?(?=AWAY MATCHES|All|$)'
        away_section_pattern = r'AWAY MATCHES.*?(?=All|$)'
        
        home_section = re.search(home_section_pattern, html, re.DOTALL | re.IGNORECASE)
        away_section = re.search(away_section_pattern, html, re.DOTALL | re.IGNORECASE)
        ...
        # Extract matches from HOME section
        if home_section:
            home_html = home_section.group(0)
            # Extract match data from table-like structure
            home_matches_data = self.extract_matches_from_section(home_html, team_name, 'H')
            home_matches.extend(home_matches_data)
        
        # Extract matches from AWAY section  
        if away_section:
            away_html = away_section.group(0)
            # Extract match data from table-like structure
            away_matches_data = self.extract_matches_from_section(away_html, team_name, 'A')
            away_matches.extend(away_matches_data)
        
        # Also extract from combined "All" section for backup
        all_pattern = r'All.*?(?=\s*Legend:|$)'
        all_section = re.search(all_pattern, html, re.DOTALL | re.IGNORECASE)
        
        if all_section and (len(home_matches) < 3 or len(away_matches) < 3):
            all_html = all_section.group(0)
            # Try to extract all matches and separate by venue
            all_matches = self.get_last_matches(html, team_name, count=20)
        ...
        home_matches.sort(key=lambda x: self.parse_date_to_sortable(x.date), reverse=True)
        away_matches.sort(key=lambda x: self.parse_date_to_sortable(x.date), reverse=True)
        
        return {
            "home": home_matches[:6],  # Last 6 home matches
            "away": away_matches[:6],  # Last 6 away matches
            "all": home_matches[:6] + away_matches[:6]  # Combined for backward compatibility
        }
    ...
    def display_analysis(self, home_team: str, away_team: str,
                        ...
                        h2h_matches: List[H2HMatch], factors: MatchFactors,
                        home_home_metrics: Dict = None, away_away_metrics: Dict = None):
        """Display comprehensive analysis with improved formatting including home/away analysis"""
        ...
        # Home/Away Specific Analysis - ENHANCED WITH PROPER METRICS
        print(f"🏠🏃 HOME/AWAY SPECIFIC ANALYSIS")
        print(f"{'Team':<20} {'Home Form':<12} {'Away Form':<12} {'Home Goals':<12} {'Away Goals':<12} {'Key Stats'}")
        print("-" * 85)
        
        # Home team analysis
        home_home_form = home_home_metrics.get('home_form_rating', 0.5) if home_home_metrics else 0.5
        home_away_form = home_home_metrics.get('away_form_rating', 0.5) if home_home_metrics else 0.5
        home_home_goals = home_home_metrics.get('home_scoring_rate', home_metrics.home_scoring_rate) if home_home_metrics else home_metrics.home_scoring_rate
        home_away_goals = home_home_metrics.get('away_scoring_rate', home_metrics.goal_scoring_rate) if home_home_metrics else home_metrics.goal_scoring_rate
        
        # Extract home team's home performance from matches
        home_home_matches_count = len([m for m in home_matches if m.venue == 'H'])
        home_away_matches_count = len([m for m in home_matches if m.venue == 'A'])
        
        if home_home_matches_count > 0:
            home_home_wins = len([m for m in home_matches if m.venue == 'H' and m.result == 'W'])
            home_home_form_actual = home_home_wins / home_home_matches_count
            home_home_goals_actual = sum([m.goals_scored for m in home_matches if m.venue == 'H']) / home_home_matches_count
        else:
            home_home_form_actual = 0.5
            home_home_goals_actual = home_metrics.goal_scoring_rate
            
        if home_away_matches_count > 0:
            home_away_wins = len([m for m in home_matches if m.venue == 'A' and m.result == 'W'])
            home_away_form_actual = home_away_wins / home_away_matches_count
            home_away_goals_actual = sum([m.goals_scored for m in home_matches if m.venue == 'A']) / home_away_matches_count
        else:
            home_away_form_actual = 0.5
            home_away_goals_actual = home_metrics.goal_scoring_rate
        
        # Away team analysis
        away_home_matches_count = len([m for m in away_matches if m.venue == 'H'])
        away_away_matches_count = len([m for m in away_matches if m.venue == 'A'])
        
        if away_home_matches_count > 0:
            away_home_wins = len([m for m in away_matches if m.venue == 'H' and m.result == 'W'])
            away_home_form_actual = away_home_wins / away_home_matches_count
            away_home_goals_actual = sum([m.goals_scored for m in away_matches if m.venue == 'H']) / away_home_matches_count
        else:
            away_home_form_actual = 0.5
            away_home_goals_actual = away_metrics.goal_scoring_rate
            
        if away_away_matches_count > 0:
            away_away_wins = len([m for m in away_matches if m.venue == 'A' and m.result == 'W'])
            away_away_form_actual = away_away_wins / away_away_matches_count
            away_away_goals_actual = sum([m.goals_scored for m in away_matches if m.venue == 'A']) / away_away_matches_count
        else:
            away_away_form_actual = 0.5
            away_away_goals_actual = away_metrics.goal_scoring_rate
        
        # Display home team stats
        home_key_stats = []
        if home_home_form_actual > 0.7:
            home_key_stats.append("Strong at home")
        elif home_home_form_actual < 0.3:
            home_key_stats.append("Weak at home")
        if home_away_form_actual > 0.6:
            home_key_stats.append("Good away")
        elif home_away_form_actual < 0.3:
            home_key_stats.append("Poor away")
        if home_home_goals_actual > 2.0:
            home_key_stats.append("High home scoring")
        
        home_stats_str = " | ".join(home_key_stats[:2]) if home_key_stats else "Balanced"
        
        print(f"{home_display[:18]:<20} {home_home_form_actual:>6.1%} ({home_home_matches_count}) {home_away_form_actual:>6.1%} ({home_away_matches_count}) {home_home_goals_actual:>8.2f} {home_away_goals_actual:>8.2f} {home_stats_str}")
        
        # Display away team stats
        away_key_stats = []
        if away_home_form_actual > 0.7:
            away_key_stats.append("Strong at home")
        elif away_home_form_actual < 0.3:
            away_key_stats.append("Weak at home")
        if away_away_form_actual > 0.6:
            away_key_stats.append("Good away")
        elif away_away_form_actual < 0.3:
            away_key_stats.append("Poor away")
        if away_away_goals_actual > 2.0:
            away_key_stats.append("High away scoring")
        
        away_stats_str = " | ".join(away_key_stats[:2]) if away_key_stats else "Balanced"
        
        print(f"{away_display[:18]:<20} {away_home_form_actual:>6.1%} ({away_home_matches_count}) {away_away_form_actual:>6.1%} ({away_away_matches_count}) {away_home_goals_actual:>8.2f} {away_away_goals_actual:>8.2f} {away_stats_str}")
        
        # Add match-specific insights
        print(f"\n🎯 MATCH CONTEXT:")
        print(f"   • {home_display[:15]} playing at HOME: {home_home_form_actual:.0%} win rate, {home_home_goals_actual:.1f} goals/game")
        print(f"   • {away_display[:15]} playing AWAY: {away_away_form_actual:.0%} win rate, {away_away_goals_actual:.1f} goals/game")
        
        # Highlight key advantages
        if home_home_form_actual > away_away_form_actual + 0.2:
            print(f"   ✅ HOME ADVANTAGE: {home_display[:12]} much stronger at home than {away_display[:12]} away")
        elif away_away_form_actual > home_home_form_actual + 0.2:
            print(f"   ⚠️  AWAY STRENGTH: {away_display[:12]} strong away form vs {home_display[:12]} home form")
        
        if home_home_goals_actual > away_away_goals_actual + 0.5:
            print(f"   ⚽ GOAL ADVANTAGE: {home_display[:12]} scores {home_home_goals_actual:.1f} at home vs {away_display[:12]}'s {away_away_goals_actual:.1f} away")
        elif away_away_goals_actual > home_home_goals_actual + 0.5:
            print(f"   ⚽ AWAY THREAT: {away_display[:12]} scores {away_away_goals_actual:.1f} away vs {home_display[:12]}'s {home_home_goals_actual:.1f} at home")
        
        print()
        
        # Recent Form Analysis - IMPROVED FORMATTING
        print(f"📊 RECENT FORM ANALYSIS")
        print(f"{'Team':<20} {'Last 6':<12} {'Key Performances'}")
        print("-" * 70)
        ...
        if len(home_notable_str) > 50:
            home_notable_str = home_notable_str[:47] + "..."
        
        print(f"{home_display[:18]:<20} {''.join(home_results):<12} {home_notable_str}")
        ...
        # Truncate if too long
        if len(away_notable_str) > 50:
            away_notable_str = away_notable_str[:47] + "..."
        
        print(f"{away_display[:18]:<20} {''.join(away_results):<12} {away_notable_str}")
        print()
        ...
        key_metrics = [
            ...
            ('Home/Away Form', f"{home_home_form_actual:.0%}" if 'home_home_form_actual' in locals() else "N/A", 
                              f"{away_away_form_actual:.0%}" if 'away_away_form_actual' in locals() else "N/A"),
        ...
        if decisions.get('value_bets'):
            print(f"💎 VALUE OPPORTUNITIES")
            for bet in decisions['value_bets'][:2]:
    ...
    def main(self, url: str, actual_result: Optional[str] = None):
        ...
        print(f"\n{'🔍 ENHANCED FOOTBALL ANALYZER WITH LEARNING 🔍'.center(70)}")
        print("─" * 70)
        ...
        home_all_matches_data = self.extract_home_away_matches(html, home_team)
        home_matches = home_all_matches_data["all"] if home_all_matches_data["all"] else self.get_last_matches(html, home_team, competition, count=8)
        home_home_matches = home_all_matches_data["home"]
        home_away_matches = home_all_matches_data["away"]
        print(f"   Found {len(home_matches)} matches ({len(home_home_matches)} home, {len(home_away_matches)} away)")
        
        print(f"📈 Analyzing {away_team}...")
        away_all_matches_data = self.extract_home_away_matches(html, away_team)
        away_matches = away_all_matches_data["all"] if away_all_matches_data["all"] else self.get_last_matches(html, away_team, competition, count=8)
        away_home_matches = away_all_matches_data["home"]
        away_away_matches = away_all_matches_data["away"]
        print(f"   Found {len(away_matches)} matches ({len(away_home_matches)} home, {len(away_away_matches)} away)")
        ...
        home_metrics = self.decision_engine.compute_metrics_from_matches(
            ...
            home_home_matches, home_away_matches
        )
        away_metrics = self.decision_engine.compute_metrics_from_matches(
            ...
            away_home_matches, away_away_matches
        )
        
        # Calculate home/away specific metrics
        home_home_metrics = self.decision_engine.calculate_home_away_specific_metrics(
            home_team, home_home_matches, home_away_matches
        )
        away_away_metrics = self.decision_engine.calculate_home_away_specific_metrics(
            away_team, away_home_matches, away_away_matches
        )
        ...
        probabilities = self.decision_engine.calculate_probabilities_with_learning(
            home_metrics, away_metrics, core_analysis, match_summary_prelim, self.factors,
            home_home_metrics, away_away_metrics, league
        )
        ...
        # Make decisions including home/away analysis
        decisions = self.decision_engine.make_decisions(
            ...
            home_home_metrics, away_away_metrics
        )
        ...
        # Display analysis with home/away data
        self.display_analysis(
            home_team, away_team, home_metrics, away_metrics,
            probabilities, goal_markets, competition, home_matches, away_matches,
            decisions, core_analysis, match_summary, h2h_matches, self.factors,
            home_home_metrics, away_away_metrics
        )
        
        # Record for auto-verification
        match_data = {
            'home_team': home_team,
            'away_team': away_team,
        ...
        self.verification_system.record_prediction_for_verification(match_data, url)
        
        # If actual result provided, record for learning
        if actual_result:
            match_id = self.generate_match_id(url)
...
if __name__ == "__main__":
    ...
    # Start auto-verification system
    analyzer = EnhancedForebetAnalyzer()
    analyzer.verification_system.start_auto_verification()
    
    if len(sys.argv) < 2:
        print("Usage: python jiv2.py <forebet_url> [actual_result]")
        print("Example: python jiv2.py https://www.forebet.com/en/football/matches/team1-team2-12345")
        print("Optional: Add H/A/D for actual result to record for learning")
        sys.exit(1)
